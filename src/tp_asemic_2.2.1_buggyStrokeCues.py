...
class Structure:    
    ...
    def draw(self,app):
        ready = []
        for currStroke in self.strokes:
            if not currStroke.isReady:
                currStroke.draw(app,'gray')
            else:
                ready.append(currStroke)
        for readyStroke in ready:
            readyStroke.draw(app,'black')
    
    def update(self,app):
        for currStroke in self.strokes:
            currStroke.translate(app)
...
class Stroke:  
    ...
    def translate(self,app):
        ...
        ptStartX = ptStartX + (dx * self.scalar) 
        ptStartY = ptStartY + (dy * self.scalar) 
        for i in range(len(self.path)-1): # iterate over segments in path, add dx and dy
            ptEndX, ptEndY = self.path[i+1]
            ptEndX = (ptEndX + dx) * self.scalar 
            ptEndY = (ptEndY + dy) * self.scalar 
            translatedPath = ((ptStartX+tempGridOffset, ptStartY+tempGridOffset, ptEndX+tempGridOffset, ptEndY+tempGridOffset))
            ptStartX = ptEndX # move to next segment
            ptStartY = ptEndY 
        ...
        self.translatedPath = translatedPath
    
    def draw(self,app,strokeCol):
        print(f'selfPath {self.path} selfTransPath {self.translatedPath}')
        drawLine(*self.translatedPath,lineWidth=app.strucStrokeWidth,fill=strokeCol)
...
def redrawAll(app): 
    ...
    app.S.update(app)   # update line segments for drawing
